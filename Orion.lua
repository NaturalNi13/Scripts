...
local function AddDraggingFunctionality(DragPoint, Main)
	pcall(function()
		...
		-- Handle InputChanged for both mouse and touch movement
...
function OrionLib:MakeWindow(WindowConfig)
	...
	function TabFunction:MakeTab(TabConfig)
		...
		local function GetElements(ItemParent)
			...
			function ElementFunction:AddBigTextBox(TextBoxConfig)
    TextBoxConfig = TextBoxConfig or {}
    TextBoxConfig.Name = TextBoxConfig.Name or "BigTextBox"
    TextBoxConfig.Callback = TextBoxConfig.Callback or function() end
    TextBoxConfig.Placeholder = TextBoxConfig.Placeholder or "Enter text here..."

    local BigTextBox = {}

    -- Create the TextBox element
    local InputTextBox = SetProps(MakeElement("TextBox"), {
        Size = UDim2.new(1, 0, 1, 0),
        TextWrapped = true, -- Ensure the text wraps within the box
        MultiLine = true, -- Allow multiple lines of text
        Text = "", -- Default text
        PlaceholderText = TextBoxConfig.Placeholder, -- Placeholder text
        Font = Enum.Font.GothamBold, -- TextBox font
        TextSize = 15, -- TextBox font size
        ClearTextOnFocus = false, -- Keep the text when the box is focused
    ...
    local TextBoxFrame = AddThemeObject(SetChildren(SetProps(MakeElement("RoundFrame", Color3.fromRGB(255, 255, 255), 0, 5), {
        Size = UDim2.new(1, 0, 0, 100), -- Adjust height for multiple lines
        Parent = ItemParent
    }), {
        AddThemeObject(SetProps(InputTextBox, {
            Size = UDim2.new(1, -12, 1, 0),
            Position = UDim2.new(0, 12, 0, 0),
            TextColor3 = Color3.fromRGB(0, 0, 0), -- Black text color
            Name = "InputBoxContent"
        }), "Text"),
        AddThemeObject(MakeElement("Stroke"), "Stroke")
    }), "Second")

    -- Optional: Add animations if needed (e.g., for visual feedback on focus)
    AddConnection(InputTextBox.Focused, function()
    ...
    AddConnection(InputTextBox.FocusLost, function()
        TweenService:Create(TextBoxFrame, TweenInfo.new(0.25, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
            BackgroundColor3 = OrionLib.Themes[OrionLib.SelectedTheme].Second
        }):Play()
    end)

    -- Function to set the TextBox text programmatically
    function BigTextBox:SetText(Text)
        InputTextBox.Text = Text
    end

    -- Override the Enter key behavior to create a new line instead of submitting
    AddConnection(InputTextBox.FocusLost, function()
        -- No action needed for FocusLost, no callback submission
    end)

    -- Custom handling of the Enter key to add a new line instead of losing focus
    AddConnection(InputTextBox.InputBegan, function(input)
        if input.UserInputType == Enum.UserInputType.Keyboard and input.KeyCode == Enum.KeyCode.Return then
            -- Simulate pressing "Enter" to add a new line
